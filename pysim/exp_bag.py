...
with open('conf/rigidcloth/bag_placement/start.json','r') as f:
	config = json.load(f)
...
def reset_sim(sim, epoch):
	...
	print(sim.obstacles[1].curr_state_mesh.dummy_node.x)

def get_loss(sim):
    loss = 0
    for i in range(len(handles)):
        loss += torch.norm(sim.obstacles[1].curr_state_mesh.dummy_node.x[3:] - (sim.cloths[0].mesh.nodes[i].x))**2
    loss /= len(handles)
    return loss
