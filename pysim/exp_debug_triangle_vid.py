...
def get_loss_per_iter(sim, epoch, sim_iter):
    ...
    ref_image = mpimg.imread('demo_cloth_fold/%03d.jpg'%sim_iter)
...
def run_sim(steps, sim, net, epoch):
    ...
    for step in range(steps):
        ...
        obs_friction = 0.3 + net_output[-1]
        stiffness_mult = 1 + torch.clamp(net_output[-2], -0.5, 1000)
        grav_offset = -2 + net_output[-2]
        net_output = net_output[:-2]
        if step == 0 and epoch > 30:
            sim.gravity = torch.Tensor([0,0,grav_offset])
            orig = sim.cloths[0].materials[0].stretching
            sim.cloths[0].materials[0].stretching = orig*stiffness_mult
            sim.obs_friction = obs_friction
            print("friction, grav", obs_friction, grav_offset, stiffness_mult)
        for i in range(len(handles)):
...
with open(out_path+('/log%s.txt'%timestamp),'w',buffering=1) as f:
    ...
    net = Net(len(handles)*6 + 1, len(handles)*3 + 3)
