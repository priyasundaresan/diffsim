...
#handles += [163, 198]
#handles = [41,44,103,106]
handles = [36,103,105,106,113,150]
...
with open('conf/rigidcloth/clothing/tshirt_start_pinned.json','r') as f:
	config = json.load(f)

device = torch.device("cuda:0")
# PYTORCH3D
#ref_verts, ref_faces, _ = load_obj("meshes/rigidcloth/clothing/crewneck_3d_final.obj")
ref_verts, ref_faces, _ = load_obj("meshes/rigidcloth/clothing/crewneck_3d_final_half.obj")
...
def get_loss(sim):
    ...
    #    vertex_loss = torch.norm(ref_verts[i].cpu()-(sim.cloths[0].mesh.nodes[i].x))**2
    #    vertex_loss *= 8 if i in handles else 1
    #    loss += vertex_loss
    #loss /= node_number
