...
with open('conf/rigidcloth/clothing/start.json','r') as f:
	config = json.load(f)

ref = np.load('conf/rigidcloth/clothing/ref_mesh.npy')
...
def get_loss(sim):
    ...
    #for i in handles:
        vertex_loss = torch.norm(ref[i]-(sim.cloths[0].mesh.nodes[i].x))**2
